...
(function() {
    ...
    // ========== KONFIGURASI LEVEL ==========
    const LEVEL_CONFIG = {
        easy: { name: 'EASY', initialSpeed: 2.5, obstacleSpawnRate: 300, obstacleChance: 0.15, maxSpeed: 16 },
        medium: { name: 'MEDIUM', initialSpeed: 3, obstacleSpawnRate: 200, obstacleChance: 0.25, maxSpeed: 18 },
        hard: { name: 'HARD', initialSpeed: 3.5, obstacleSpawnRate: 120, obstacleChance: 0.4, maxSpeed: 20 }
    };
    ...
    async function initGame() {
        ...
        function resizeCanvas() {
            ...
            const bottomSpace = Math.max(safeBottom, 50);
            canvasHeight = window.innerHeight - dashboardHeight - bottomSpace;
        ...
        function spawnEnemy() {
            ...
            enemies.push({
                x: randomX, y: -enemyHeight, width: enemyWidth, height: enemyHeight,
                type: { name: enemy.name, color: enemy.color, agresif: enemy.agresif, speedBonus: enemy.speedBonus, points: enemy.points },
                image: enemy.image, agresifOffset: 0
            });
        }

        function spawnObstacle() {
            const stoneImg = gameImages.stone;
            const dims = getImageDimensions(stoneImg, 45);
            obstacles.push({
                x: Math.random() * (canvasWidth - dims.width - 40) + 20,
                y: -dims.height, width: dims.width, height: dims.height, image: stoneImg
            });
        }

        function spawnPowerup() {
            powerups.push({ x: Math.random() * (canvasWidth - 45 - 40) + 20, y: -45, size: 40 });
        }

        function spawnLifePowerup() {
            lifePowerups.push({ x: Math.random() * (canvasWidth - 45 - 40) + 20, y: -45, size: 40 });
        }
            ...
            if (isPlayer && shieldActive && !isRespawning && invincibleFrames <= 0) {
                ...
                if (!isBlinking || Math.floor(Date.now() / 100) % 2 === 0) {
                    ...
                    if (shieldImg && shieldImg.complete && shieldImg.naturalWidth > 0) {
                        ...
                        ctx.drawImage(shieldImg, x + width / 2 - iconSize / 2, y - iconSize / 1.5, iconSize, iconSize);
                    }
                    ctx.restore();
                }
            }
        ...
        function update() {
            ...
            if (crashTextTimer > 0) crashTextTimer--;
            ...
            player.x = Math.min(Math.max(player.x, canvasWidth * 0.03), canvasWidth - player.width - canvasWidth * 0.03);

            let spawnRate = Math.max(45, 110 - Math.floor(speed * 2));
            if (frameCount % spawnRate === 0) spawnEnemy();
            if (frameCount % currentLevelConfig.obstacleSpawnRate === 0 && Math.random() < currentLevelConfig.obstacleChance) spawnObstacle();
            ...
            for (let i = 0; i < enemies.length; i++) {
                ...
                enemy.y += enemySpeed;
                if (!isRespawning && enemy.x < player.x + player.width && enemy.x + enemy.width > player.x && enemy.y < player.y + player.height && enemy.y + enemy.height > player.y) {
                    const isGameOver = handleAccident();
                ...
                if (enemy.y > canvasHeight) {
                    ...
                    enemiesPassed++;
                    if (enemiesPassed % 5 === 0 && speed < currentLevelConfig.maxSpeed) speed = Math.min(currentLevelConfig.maxSpeed, speed + 0.5);
                    updateUI();
                }
            }
            ...
            for (let i = 0; i < obstacles.length; i++) {
                ...
                if (!isRespawning && obstacles[i].x < player.x + player.width && obstacles[i].x + obstacles[i].width > player.x && obstacles[i].y < player.y + player.height && obstacles[i].y + obstacles[i].height > player.y) {
                    const isGameOver = handleAccident();
                    if (isGameOver) return;
            ...
            for (let i = 0; i < powerups.length; i++) {
                ...
                if (powerups[i].x < player.x + player.width && powerups[i].x + powerups[i].size > player.x && powerups[i].y < player.y + player.height && powerups[i].y + powerups[i].size > player.y) {
                    activateShield(15);
                    powerups.splice(i, 1);
                    i--;
                    continue;
            ...
            for (let i = 0; i < lifePowerups.length; i++) {
                ...
                if (lifePowerups[i].x < player.x + player.width && lifePowerups[i].x + lifePowerups[i].size > player.x && lifePowerups[i].y < player.y + player.height && lifePowerups[i].y + lifePowerups[i].size > player.y) {
                    if (lives < 5) lives++;
                    updateUI();
        ...
        function draw() {
            ...
            for (let enemy of enemies) drawCharacter(enemy.image, enemy.x, enemy.y, enemy.width, enemy.height, false, enemy.type);

            const showAccident = (accidentDisplayTimer > 0) && !isRespawning;
            if (showAccident) drawCharacter(player.image, lastAccidentX, lastAccidentY, player.width, player.height, true, null, true);
            else drawCharacter(player.image, player.x, player.y, player.width, player.height, true);
            drawExplosion();

            // Balon teks
            if (crashTextTimer > 0 && crashText) {
                ctx.save();
                const textX = lastAccidentX + player.width / 2;
                const textY = lastAccidentY - 25;
                ctx.font = `bold ${Math.max(14, player.width * 0.25)}px "Segoe UI"`;
                const textWidth = ctx.measureText(crashText).width;
                const bubbleWidth = textWidth + 20;
                const bubbleHeight = 30;
                const floatOffset = Math.sin(Date.now() * 0.015) * 3;
                ctx.shadowBlur = 5;
                ctx.shadowColor = 'rgba(0,0,0,0.3)';
                ctx.fillStyle = 'rgba(255, 255, 255, 0.95)';
                ctx.beginPath();
                ctx.roundRect(textX - bubbleWidth / 2, textY + floatOffset - bubbleHeight, bubbleWidth, bubbleHeight, 15);
                ctx.fill();
                ctx.beginPath();
                ctx.moveTo(textX - 8, textY + floatOffset - 5);
                ctx.lineTo(textX, textY + floatOffset + 8);
                ctx.lineTo(textX + 8, textY + floatOffset - 5);
                ctx.fill();
                ...
                ctx.fillText(crashText, textX - textWidth / 2, textY + floatOffset - 12);
                ctx.restore();
            }
        ...
        // ========== TOMBOL LEVEL DI START OVERLAY ==========
        function initLevelButtons() {
            const easyBtn = document.querySelector('.level-opt.easy');
            const mediumBtn = document.querySelector('.level-opt.medium');
            const hardBtn = document.querySelector('.level-opt.hard');
            if (easyBtn) {
                easyBtn.onclick = (e) => {
                    e.preventDefault();
            ...
            if (hardBtn) {
                hardBtn.onclick = (e) => {
                    ...
                    currentLevel = 'hard';
                    setLevel('hard');
                    if (levelSelect) levelSelect.value = 'hard';
                };
            }
        }
        initLevelButtons();
